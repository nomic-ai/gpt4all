...
## Running LLMs on CPU
The GPT4All Chat UI supports models from all newer version of `ggML`, `llama.cpp` including the `LLaMA`, `MPT` and `GPT-J` architectures. The `falcon` and `replit` architectures will soon also be supported.
